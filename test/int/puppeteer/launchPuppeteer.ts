...
export class LaunchPuppeteer implements IFixture {
	...
	public static async create(
		...
		launchConfig: any /* TODO: investigate why launch config types differ between V1 and V2 */
		...
		await launchTestAdapter(
			debugClient,
			Object.assign({}, launchConfig, { port: daPort })
		);
		const browser = await connectPuppeteer(daPort);

		const page = logCallsTo(
			await getPageByUrl(browser, launchConfig.url!),
			"PuppeteerPage"
		);
