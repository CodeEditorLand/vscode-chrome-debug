...
const EXPECTED_REASON = testSetup.isThisV1
	? "debugger_statement"
	: "breakpoint";
...
function runCommonTests(breakOnLoadStrategy: BreakOnLoadStrategy) {
	...
	teardown(() => {
		if (server) {
			server.close((err) =>
				console.log(
					"Error closing server in teardown: " + (err && err.message)
				)
			);
			server = null;
		}
	...
	suite("TypeScript Project with SourceMaps", () => {
		...
		test("Hits a breakpoint at (1,1) in a file on load", async () => {
			...
			if (breakOnLoadStrategy === "instrument") {
				...
				await dc.assertStoppedLocation(EXPECTED_REASON, {
					path: scriptPath,
					line: bpLine,
					column: bpCol,
				});
...
suite("BreakOnLoad", () => {
	...
	suite("Instrument Webpack Project", () => {
		...
		setup(function () {
			return testSetup
				.setup(this, undefined, { breakOnLoadStrategy: "instrument" })
				.then((_dc) => (dc = _dc));
		});
	...
	suite("BreakOnLoad Disabled (strategy: off)", () => {
		...
		setup(function () {
			return testSetup
				.setup(this, undefined, { breakOnLoadStrategy: "off" })
				.then((_dc) => (dc = _dc));
		});
