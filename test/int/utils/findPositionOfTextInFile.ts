...
export async function findPositionOfTextInFile(
	filePath: string,
	text: string
): Promise<Position> {
	const contentsIncludingCarriageReturns = await utils.readFileP(
		filePath,
		"utf8"
	);
	...
	if (textStartIndex >= 0) {
		...
		const textColumNumber = createColumnNumber(
			textStartIndex - (lastNewLineBeforeTextIndex + 1)
		);
