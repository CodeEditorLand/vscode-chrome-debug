	...
	suite("launch", () => {
				...
				await Promise.all([
					...
					dc.initializeRequest().then((_) => {
						return dc.attachRequest(<IAttachRequestArgs>{
							...
							webRoot: testProjectRoot,
							logFilePath:
								getDebugAdapterLogFilePath(fullTestTitle),
							logTimestamps: true,
						});
					}),
					dc.assertStoppedLocation("debugger_statement", {
		...
		test("Should throw error when launching if chrome debug port is in use", async () => {
			...
			try {
				...
				assert.fail(
					"Expected launch to throw a timeout exception, but it didn't."
				);
			} catch (err) {
				expect(err.message).to.satisfy((x: string) =>
					x.startsWith(
						"Cannot connect to runtime process, timeout after 2000 ms"
					)
				);
			} finally {
				await browser.close();
