...
async function createServerAsync(root: string): Promise<HttpOrHttpsServer> {
	...
	return await new Promise((resolve, reject) => {
		...
		server.listen(
			0,
			"127.0.0.1",
			function (this: HttpOrHttpsServer, error?: any) {
				if (error) {
					reject(error);
				} else {
...
async function closeServer(server: HttpOrHttpsServer): Promise<void> {
	...
	await new Promise((resolve, reject) => {
		server.close((error?: any) => {
			if (error) {
				logger.log(
					"Error closing server in teardown: " +
						(error && error.message)
				);
				reject(error);
			} else {
...
export class LaunchWebServer implements IFixture {
	private constructor(
		...
		private readonly _testSpec: TestProjectSpec
	) {}

	public static async launch(
		testSpec: TestProjectSpec
	): Promise<LaunchWebServer> {
		return new LaunchWebServer(
			await createServerAsync(testSpec.props.webRoot),
			testSpec
		);
	}
	...
	public get launchConfig(): ILaunchRequestArgs {
		return Object.assign({}, this._testSpec.props.launchConfig, {
...
export class ProvideStaticUrl implements IFixture {
	public constructor(
		...
		private readonly testSpec: TestProjectSpec
	) {}
