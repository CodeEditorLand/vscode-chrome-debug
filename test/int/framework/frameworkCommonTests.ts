...
import {
	puppeteerTest,
	PuppeteerTestContext,
} from "../puppeteer/puppeteerSuite";
...
export class FrameworkTestSuite {
	...
	testPageReloadBreakpoint(bpLabel: string) {
		return puppeteerTest(
			...
			async (context, page) => {
				...
				await debugClient.assertStoppedLocation(
					"breakpoint",
					bpLocation
				);
	...
	testStepIn(bpLabelStop: string, bpLabelStepIn: string) {
		return puppeteerTest(
			...
			async (_context, page) => {
				const location =
					this.suiteContext.breakpointLabels.get(bpLabelStop);
				const stepInLocation =
					this.suiteContext.breakpointLabels.get(bpLabelStepIn);
				...
				await this.suiteContext.debugClient.assertStoppedLocation(
					"breakpoint",
					location
				);
				await this.pausedWizard.waitAndConsumePausedEvent(() => {});
				...
				const stopOnStep =
					this.suiteContext.debugClient.assertStoppedLocation(
						"step",
						stepInLocation
					);
	...
	testStepOver(bpLabel: string) {
		return puppeteerTest(
			...
			async (_context, page) => {
				const location =
					this.suiteContext.breakpointLabels.get(bpLabel);
	...
	testStepOut(bpLabelStop: string, bpLabelStepOut: string) {
		return puppeteerTest(
			...
			async (_context, page) => {
				const location =
					this.suiteContext.breakpointLabels.get(bpLabelStop);
				const stepOutLocation =
					this.suiteContext.breakpointLabels.get(bpLabelStepOut);
 ...
 * @param bpLabel Label for the breakpoint to set
 */
export function testBreakOnLoad(
	...
	return test(testTitle, async () => {
		...
		try {
			const breakpointLabels = await loadProjectLabels(
				testSpec.props.webRoot
			);
