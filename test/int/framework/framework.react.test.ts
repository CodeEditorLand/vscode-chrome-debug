...
puppeteerSuite("React Framework Tests", TEST_SPEC, (suiteContext) => {
	suite("Common Framework Tests", () => {
		...
		frameworkTests.testStepOut(
			"react_Counter_increment",
			"react_Counter_stepOut"
		);
		frameworkTests.testStepIn(
			"react_Counter_stepInStop",
			"react_Counter_stepIn"
		);
	});

	suite("React specific tests", () => {
		puppeteerTest(
			"Should hit breakpoint in .jsx file",
			suiteContext,
			async (_context, page) => {
				const pausedWizard = suiteContext.launchProject!.pausedWizard;

				const location = suiteContext.breakpointLabels.get(
					"react_Counter_increment"
				);
				...
				await suiteContext.debugClient.assertStoppedLocation(
					"breakpoint",
					location
				);
		...
		puppeteerTest(
			...
			async (_context, page) => {
				...
				await setConditionalBreakpoint(
