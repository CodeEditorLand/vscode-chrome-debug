...
export class TestProjectSpec {
	 ...
	 */
	constructor(
		props: ProjectSpecProps,
		public readonly staticUrl?: string
	) {
		const outFiles = props.outFiles || [
			path.join(props.projectRoot, "out"),
		];
	...
	public static fromTestPath(
		...
		const projectAbsolutePath = path.join(
			...[DATA_ROOT].concat(pathComponents)
		);
		const projectSrc = path.join(projectAbsolutePath, sourceDir);
		let props: ProjectSpecProps = {
			projectRoot: projectAbsolutePath,
			projectSrc,
		};
...
export class ReassignableFrameworkTestContext implements FrameworkTestContext {
	private _wrapped: FrameworkTestContext =
		new NotInitializedFrameworkTestContext();
...
export class NotInitializedFrameworkTestContext
	implements FrameworkTestContext
{
	public get testSpec(): TestProjectSpec {
