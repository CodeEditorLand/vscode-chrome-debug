...
export class Position implements IEquivalenceComparable {
	public static readonly origin = new Position(
		createLineNumber(0),
		createColumnNumber(0)
	);
	...
	public static isBetween(
		start: Position,
		maybeInBetween: Position,
		end: Position
	): boolean {
		return (
			!maybeInBetween.doesAppearBefore(start) &&
			!end.doesAppearBefore(maybeInBetween)
		);
	}
	...
	public isOrigin(): boolean {
		return (
			this.lineNumber === 0 &&
			(this.columnNumber === undefined || this.columnNumber === 0)
		);
	}
	...
	public doesAppearBefore(right: Position): boolean {
		return (
			...
			(this.lineNumber === right.lineNumber &&
				this.columnNumber < right.columnNumber)
		);
	}
