...
export interface IValidatedMap<K, V> extends Map<K, V> {
	...
	setAndIgnoreDuplicates(
		key: K,
		value: V,
		comparer?: ValueComparerFunction<V>
	): this;
}
...
export class ValidatedMap<K, V> implements IValidatedMap<K, V> {
	...
	constructor(
		initialContents?: Map<K, V> | Iterable<[K, V]> | ReadonlyArray<[K, V]>
	) {
	...
	public setAndIgnoreDuplicates(
		...
		if (
			existingValueOrUndefined !== undefined &&
			!comparer(existingValueOrUndefined, value)
		) {
			breakWhileDebugging();
			throw new Error(
