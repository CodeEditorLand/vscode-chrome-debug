...
export class ValidatedMultiMap<K, V> {
	...
	private constructor(
		private readonly _wrappedMap: IValidatedMap<K, IValidatedSet<V>>
	) {}
	...
	public static withContents<K, V>(
		initialContents:
			| Map<K, Set<V>>
			| Iterable<[K, Set<V>]>
			| ReadonlyArray<[K, Set<V>]>
	): ValidatedMultiMap<K, V> {
		const elements = Array.from(initialContents).map(
			(element) =>
				<[K, IValidatedSet<V>]>[
					element[0],
					new ValidatedSet(element[1]),
				]
		);
		return this.usingCustomMap(
	...
	public getOr(key: K, elementDoesntExistAction: () => Set<V>): Set<V> {
		return this._wrappedMap.getOr(
			key,
			() => new ValidatedSet(elementDoesntExistAction())
		);
	}
