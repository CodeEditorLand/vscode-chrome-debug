...
export class MethodsCalledLoggerConfiguration
	implements IMethodsCalledLoggerConfiguration
{
	public constructor(
...
export class MethodsCalledLogger<T extends object> {
				...
				if (typeof originalPropertyValue === "function") {
					return (...args: any) => {
						...
						try {
							...
							const result = originalPropertyValue.apply(
								target,
								args
							);
							if (!result || !result.then) {
								...
								if (result === target) {
									...
									return this._configuration.customizeResult(
										propertyKey,
										args,
										result
									);
	...
	private printMethodCall(
		propertyKey: PropertyKey,
		methodCallArguments: any[]
	): string {
		return `${this._objectToWrapName}.${String(
			propertyKey
		)}(${this.printArguments(methodCallArguments)})`;
	}

	private printMethodResponse(
		outcome: Outcome,
		resultOrException: unknown
	): string {
		return `${
			outcome === Outcome.Succesful ? "->" : "threw"
		} ${this.printObject(resultOrException)}`;
	}
	...
	private getTestFileAndLine(): string {
		...
		if (stack) {
			...
			const testCaseLine = stackLines.find(
				(line) => line.indexOf("test.ts") >= 0
			);
			if (testCaseLine) {
				const filenameAndLine = testCaseLine.lastIndexOf(path.sep);
				if (filenameAndLine >= 0) {
					const fileNameAndLineNumber = testCaseLine.substring(
						filenameAndLine + 1,
						testCaseLine.length - 2
					);
	...
	private logCallStart(
		...
		const message = `START            ${callId}: ${getTestFileAndLine}${this.printMethodCall(
			propertyKey,
			methodCallArguments
		)}`;
	...
	private logSyncPartFinished(
		...
		const getTestFileAndLine = this.getTestFileAndLine();
		const message = `PROMISE-RETURNED ${callId}: ${getTestFileAndLine}${this.printMethodCall(
			propertyKey,
			methodCallArguments
		)}`;
		logger.verbose(message);
	...
	private logCall(
		...
		const message = `${endPrefix}${this.printMethodCall(
			propertyKey,
			methodCallArguments
		)} ${this.printMethodSynchronicity(
			synchronicity
		)}  ${this.printMethodResponse(outcome, resultOrException)}`;
		logger.verbose(message);
	}

	private printArguments(methodCallArguments: any[]): string {
		return methodCallArguments
			.map((methodCallArgument) => this.printObject(methodCallArgument))
			.join(", ");
	}

	private printObject(objectToPrint: unknown): string {
		const description = printTopLevelObjectDescription(objectToPrint);
		const printedReduced = _.reduce(
			Array.from(this._configuration.replacements),
			(text, replacement) =>
