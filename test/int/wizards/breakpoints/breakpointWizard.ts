...
export class BreakpointWizard {
	private _state: IBreakpointSetOrUnsetState = new BreakpointUnsetState(
		this,
		this._internal,
	...
	public async setThenWaitForVerifiedThenValidate(): Promise<BreakpointWizard> {
		...
		if (isThisV2) {
			// this will hang indefinetly on V1 in certain cases, particularly hit count bps with invalid condiditions
	...
	public async assertIsHitThenResumeWhen(
		...
		await this._state.assertIsHitThenResumeWhen(
...
export type VSCodeActionWhenHit = RemoveProperty<
	DebugProtocol.SourceBreakpoint,
	"line" | "column"
>;

export type ChangeBreakpointWizardState = (
	newState: IBreakpointSetOrUnsetState
) => void;
...
class BreakpointSetState implements IBreakpointSetOrUnsetState {
	 ...
	 */
	public assertIsHitThenResumeWhen(
		lastActionToMakeBreakpointHit: () => Promise<void>,
		verifications: IVerifications
	): Promise<void> {
		return this._internal.assertIsHitThenResumeWhen(
			this._breakpoint,
			lastActionToMakeBreakpointHit,
			verifications
		);
	...
	public assertIsHitThenResume(verifications: IVerifications): Promise<void> {
		return this._internal.assertIsHitThenResume(
			this._breakpoint,
			verifications
		);
	}
...
export class BreakpointUnsetState implements IBreakpointSetOrUnsetState {
	...
	public async waitUntilVerified(): Promise<void> {
		throw new Error(
			`Can't expect an unset breakpoint to ever become verified`
		);
	}
}
