...
import {
	VariablesWizard,
	IExpectedVariables,
} from "../../variables/variablesWizard";
...
export class BreakpointsAssertions {
	private readonly _variablesWizard = new VariablesWizard(
		this._internal.client
	);
	...
	public async waitUntilVerified(
		...
		await waitUntilReadyWithTimeout(
			() => this.currentBreakpointsMapping.get(breakpoint).verified
		);
	}
	...
	public async assertIsHitThenResume(
		...
		const stackTraceFrames = (
			await stackTrace(
				this._internal.client,
				verifications.stackFrameFormat
			)
		).stackFrames;
		...
		if (typeof verifications.stackTrace === "string") {
			...
			assertions.assertResponseMatches(
				stackTraceFrames,
				verifications.stackTrace
			);
		} else if (typeof verifications.stackTrace === "object") {
			const assertions = new StackTraceObjectAssertions(
	...
	private assertLocationMatchesExpected(
		objectWithLocation: IObjectWithLocation,
		breakpoint: BreakpointWizard
	): void {
		if (isThisV2) {
			// Disable this completely for v1
			...
			const expectedColumNumber =
				breakpoint.boundPosition.columnNumber + 1;
			const expectedBPLocationPrinted = `${expectedFilePath}:${expectedLineNumber}:${expectedColumNumber}`;
			const actualBPLocationPrinted = `${
				objectWithLocation.source!.path
			}:${objectWithLocation.line}:${objectWithLocation.column}`;
