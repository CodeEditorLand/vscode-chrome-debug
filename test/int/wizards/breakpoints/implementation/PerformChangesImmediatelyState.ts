...
export class PerformChangesImmediatelyState
	implements IBreakpointsBatchingStrategy
{
	private readonly _idToBreakpoint = new ValidatedMap<
		number,
		BreakpointWizard
	>();
	...
	public async set(breakpointWizard: BreakpointWizard): Promise<void> {
		if (this.currentBreakpointsMapping.has(breakpointWizard)) {
			throw new Error(
				`Can't set the breakpoint: ${breakpointWizard} because it's already set`
			);
		}
	...
	public async unset(breakpointWizard: BreakpointWizard): Promise<void> {
		if (!this.currentBreakpointsMapping.has(breakpointWizard)) {
			throw new Error(
				`Can't unset the breakpoint: ${breakpointWizard} because it is not set`
			);
		}

		const remainingBreakpoints = this.currentBreakpoints().filter(
			(bp) => bp !== breakpointWizard
		);
		await this._internal.sendBreakpointsToClient(
	...
	public onBreakpointStatusChange(
		breakpointStatusChanged: BreakpointStatusChangedWithId
	): void {
		const breakpoint = this._idToBreakpoint.get(
			breakpointStatusChanged.breakpoint.id
		);
	...
	public async waitUntilVerified(
	...
	public async assertIsHitThenResumeWhen(
		...
		await this._breakpointsAssertions.assertIsHitThenResumeWhen(
			breakpoint,
	...
	public async assertIsHitThenResume(
		...
		await this._breakpointsAssertions.assertIsHitThenResume(
			breakpoint,
			verifications
		);
	}
