...
export class StackTraceStringAssertions {
	...
	public assertResponseMatches(
		...
		stackTraceFrames.forEach((frame) => {
			...
			const expectedSourceNameAndLine = ` [${frame.source!.name}] Line ${
				frame.line
			}`;
			expect(
				frame.name,
				"Expected the formatted name to match the source name and line supplied as individual attributes"
			).to.endsWith(expectedSourceNameAndLine);
		});

		const formattedExpectedStackTrace =
			trimWhitespaceAndComments(expectedString);
	...
	private applyIgnores(
		...
		const ignoreFunctionName = findLineNumber(
			formattedExpectedStackTrace,
			formattedExpectedStackTrace.indexOf(ignoreFunctionNameText)
		);
		if (ignoreFunctionName >= 0) {
			...
			ignoredFrame.name = `${ignoreFunctionNameText} [${
				ignoredFrame.source!.name
			}] Line ${ignoredFrame.line}`;
		}
	}
