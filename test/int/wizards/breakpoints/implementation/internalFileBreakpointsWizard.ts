...
export type BreakpointWithId = MakePropertyRequired<
...
export interface IBreakpointsBatchingStrategy {
	...
	onBreakpointStatusChange(
		breakpointStatusChanged: BreakpointStatusChangedWithId
	): void;
}

export type CurrentBreakpointsMapping = ValidatedMap<
	BreakpointWizard,
	BreakpointWithId
>;
...
export class InternalFileBreakpointsWizard {
	...
	private _state: IBreakpointsBatchingStrategy =
	...
	public async breakpoint(options: {
		...
		const position = await findPositionOfTextInFile(
			this.filePath,
			options.text
		);
	...
	public async batch<T>(
		batchAction: (
			fileBreakpointsWizard: FileBreakpointsWizard
		) => PromiseOrNot<T>
	): Promise<T> {
		const batchingUpdates = new BatchingUpdatesState(
			this,
			this._state.currentBreakpointsMapping
		);
	...
	public async sendBreakpointsToClient(
		update: BreakpointsUpdate
	): Promise<void> {
