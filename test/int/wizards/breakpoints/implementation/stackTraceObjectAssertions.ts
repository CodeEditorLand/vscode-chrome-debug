...
export class StackTraceObjectAssertions {
	...
	private assertSourceMatches(
		...
		if (expected == null) {
			assert.fail(
				`Source was returned for frame ${index} but none was expected`
			);
			return;
		}
		...
		if (actual == null) {
			assert.fail(
				`Source was expected for frame ${index} but none was returned`
			);
			return;
		}
		...
		if (expected.fileRelativePath) {
			...
			expect(actual.name).to.match(
				/.*VM.*/,
				`Frame ${index} source name`
			);
			expect(actual.path).to.match(
				/.*VM.*/,
				`Frame ${index} source path`
			);
			return;
		} else {
			assert.fail(
	...
	private assertFrameMatches(
		...
		if (testSetup.isThisV1 && expected.presentationHint === "normal") {
			...
			expect(actual.presentationHint, `Frame ${index} presentationHint`)
				.to.be.undefined;
		} else {
	...
	private assertResponseMatchesFrames(
		...
		// Check array length
		expect(actualFrames.length).to.equal(
