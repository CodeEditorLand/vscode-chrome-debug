...
export class BreakpointsUpdater {
	...
	public async update(update: BreakpointsUpdate): Promise<void> {
		...
		const vsCodeBps = updatedBreakpoints.map((bp) =>
			this.toVSCodeProtocol(bp)
		);
		...
		const breakpointToStatus = new ValidatedMap<
			BreakpointWizard,
			BreakpointWithId
		>(
	...
	private toVSCodeProtocol(
		...
		const actionWhenHitInformation =
			this.actionWhenHitToVSCodeProtocol(breakpoint);
		return Object.assign({}, commonInformation, actionWhenHitInformation);
	}

	private actionWhenHitToVSCodeProtocol(
		breakpoint: BreakpointWizard
	): VSCodeActionWhenHit {
		if (breakpoint.actionWhenHit instanceof AlwaysPause) {
			...
			return {
				hitCondition: breakpoint.actionWhenHit.pauseOnHitCondition,
			};
		} else {
	...
	private validateResponse(
		...
		if (!response.success) {
			throw new Error(
				`Failed to set the breakpoints for: ${this._internal.filePath}`
			);
		}
		...
		const bpsWithoutId = response.body.breakpoints.filter(
			(bp) => bp.id === undefined
		);
