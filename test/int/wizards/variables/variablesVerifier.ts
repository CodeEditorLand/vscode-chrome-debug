...
export class VariablesVerifier {
	...
	private assertVariablesPrintedAre(
		...
		const trimmedVariables = trimWhitespaceAndComments(
			expectedVariablesPrinted
		);
	...
	public assertVariablesValuesSatisfy(
		...
		const actualVariableNames = manyVariables.map(
		...
		switch (kindOfVerification) {
			case KindOfVerification.ProperSubset:
				expect(actualVariableNames).to.contain.members(
					expectedVariablesNames
				);
				break;
			case KindOfVerification.SameAndExact:
				expect(actualVariableNames).to.have.members(
					expectedVariablesNames
				);
				break;
			default:
				throw new Error(
					`Unexpected comparison algorithm: ${kindOfVerification}`
				);
		}
		...
		for (const variable of manyVariables) {
			...
			if (expectedVariablesNames.indexOf(variableName) >= 0) {
				...
				expect(kindOfVerification).to.equal(
					KindOfVerification.ProperSubset
				); // This should not happen for same elements
			}
		}
	}
}
