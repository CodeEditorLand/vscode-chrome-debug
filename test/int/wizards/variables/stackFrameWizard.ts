...
import {
	THREAD_ID,
	ExtendedDebugClient,
} from "vscode-chrome-debug-core-testsupport";
import { VariablesScopeName } from "./variablesWizard";
import {
	ValidatedSet,
	IValidatedSet,
} from "../../core-v2/chrome/collections/validatedSet";
...
export async function stackTrace(
	...
	const stackFrameFormat = _.defaultTo(
		optionalStackFrameFormat,
...
export async function topStackFrame(
	...
	const stackFrames = (await stackTrace(client, optionalStackFrameFormat))
		.stackFrames;
...
export class StackFrameWizard {
	...
	public static async topStackFrame(
		client: ExtendedDebugClient
	): Promise<StackFrameWizard> {
	...
	public async variablesOfScopes(
		...
		return Promise.all(
			scopes.map(async (scope) => {
				...
				return {
					scopeName: <VariablesScopeName>scope.name.toLowerCase(),
					variables,
				};
			})
		);
	...
	private async scopesByNames(
		...
		const scopeNamesSet = new ValidatedSet(
			manyScopeNames.map((name) => name.toLowerCase())
		);
	...
	public async scopes(): Promise<DebugProtocol.Scope[]> {
		const scopesResponse = await this._client.scopesRequest({
			frameId: this._stackFrame.id,
		});
		logger.log(
	...
	public async globalVariableNames(): Promise<IValidatedSet<string>> {
		...
		return new ValidatedSet(
			existingGlobalVariables.map((variable) => variable.name)
		);
	}

	/** Return the variables information for a particular scope of the underlying stack frame */
	public async variablesOfScope(
		scopeName: VariablesScopeName
	): Promise<DebugProtocol.Variable[]> {
		return singleElementOfArray(await this.variablesOfScopes([scopeName]))
			.variables;
	}
}
