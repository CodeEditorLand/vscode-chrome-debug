...
export type IScopeExpectedVariables =
	| ManyVariablesPropertiesPrinted
	| ManyVariablesValues;
...
export class VariablesWizard {
	...
	public async assertNewGlobalVariariablesAre(
		...
		const namesOfGlobalsToIgnore = await (
			await this.topStackFrameHelper()
		).globalVariableNames();

		// Perform an action that adds new global variables
		await actionThatAddsNewVariables();

		const globalsOnFrame = await (
			await this.topStackFrameHelper()
		).variablesOfScope("global");
		const nonIgnoredGlobals = globalsOnFrame.filter(
			(global) => !namesOfGlobalsToIgnore.has(global.name)
		);
		const expectedGlobalsTrimmed =
			trimWhitespaceAndComments(expectedGlobals);
		expect(printVariables(nonIgnoredGlobals)).to.equal(
			expectedGlobalsTrimmed
		);
	}
	 ...
	 */
	public async assertTopFrameVariablesAre(
		verifications: IExpectedVariables
	): Promise<void> {
		await this.assertStackFrameVariablesAre(
			await this.topStackFrameHelper(),
			verifications
		);
	}
	...
	public async assertStackFrameVariablesAre(
		...
		const withoutModifiersToWith = new ValidatedMap(
			_.zip(scopesWithoutModifiers, scopesWithModifiers)
		);
		const manyScopes = await stackFrame.variablesOfScopes(
			scopesWithoutModifiers
		);
		for (const scope of manyScopes) {
			const scopeNameWithModifier = withoutModifiersToWith.get(
				scope.scopeName
			)!;
			const [, modifier] = this.splitIntoScopeNameAndModifier(
				scopeNameWithModifier
			);
			switch (modifier) {
				case "":
					this.verificator.assertVariablesAre(
						scope.variables,
						verifications[scopeNameWithModifier]!
					);
					break;
				case "contains":
	...
	private splitIntoScopeNameAndModifier(
		...
		if (components.length > 2) {
			throw new Error(
				`Invalid modified scope name: ${modifiedScopeName}`
			);
		}
