...
export class PausedWizard {
	...
	private _eventsToBeConsumed: (
		| DebugProtocol.ContinuedEvent
		| DebugProtocol.StoppedEvent
	)[] = [];
	private static _clientToPausedWizard = new ValidatedMap<
		ExtendedDebugClient,
		PausedWizard
	>();
	...
	private constructor(private readonly _client: ExtendedDebugClient) {
		...
		if (isThisV2) {
			// Don't bother subscribing on v1, as v1 sends more continues than strictly necessary
			this._client.on("continued", (continued) =>
				this.onEvent(continued)
			);
		}
	}
	...
	public async waitAndConsumeResumedEvent(): Promise<void> {
		if (isThisV2) {
			await waitUntilReadyWithTimeout(
				() => this.nextEventToConsume === EventToConsume.Resumed
			);
	...
	public async waitUntilPausedOnDebuggerStatement(): Promise<void> {
		return this.waitAndConsumePausedEvent((pauseInfo) => {
			expect(pauseInfo.description).to.equal(
	...
	public async waitAndConsumePausedEvent(
		actionWithPausedInfo: (
			pausedInfo: DebugProtocol.StoppedEvent["body"]
		) => void
	): Promise<void> {
		await waitUntilReadyWithTimeout(
			() => this.nextEventToConsume === EventToConsume.Paused
		);
		const pausedEvent = <DebugProtocol.StoppedEvent>(
			this._eventsToBeConsumed[0]
		);
	...
	private logState() {
		logger.log(
			`Resume/Pause #events = ${
				this._eventsToBeConsumed.length
			}, state = ${EventToConsume[this.nextEventToConsume]}`
		);
	}
