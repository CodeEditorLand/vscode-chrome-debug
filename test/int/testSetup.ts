...
let testLaunchProps:
	| any
	| undefined; /* TODO: investigate why launch config types differ between V1 and V2 */
...
export async function setup(
	...
	port?: number,
	launchProps?: ILaunchRequestArgs
): Promise<ts.ExtendedDebugClient> {
...
export async function setupWithTitle(
	...
	if (!port) {
		...
		port = daPort ? parseInt(daPort, 10) : undefined;
	}
	...
	const debugClient = await ts.setup({
	...
	debugClient.defaultTimeout =
		DefaultTimeoutMultiplier * 10000 /*10 seconds*/;

	if (isThisV2) {
		// The logging proxy breaks lots of tests in v1, possibly due to some race conditions exposed by the extra delay
		const wrappedDebugClient = logCallsTo(
			debugClient,
			"DebugAdapterClient"
		);
