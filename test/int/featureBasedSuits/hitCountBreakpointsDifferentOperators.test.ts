...
puppeteerSuite(
	...
	(suiteContext) => {
		...
		const manyConditionsConfigurations: IConditionConfiguration[] = [
			{
				condition: "=     0",
				iterationsExpectedToPause: [],
				noMorePausesAfterwards: true,
			},
			{
				condition: "= 1",
				iterationsExpectedToPause: [1],
				noMorePausesAfterwards: true,
			},
			{
				condition: "= 2",
				iterationsExpectedToPause: [2],
				noMorePausesAfterwards: true,
			},
			{
				condition: "= 12",
				iterationsExpectedToPause: [12],
				noMorePausesAfterwards: true,
			},
			{
				condition: "> 0",
				...
				async (_context, page) => {
					...
					for (const nextIterationToPause of conditionConfiguration.iterationsExpectedToPause) {
						...
						await setStateBreakpoint.assertIsHitThenResume({
							variables: {
								local_contains: {
									iterationNumber: nextIterationToPause,
								},
							},
						});
					}
					...
					logger.log(
						`No more pauses afterwards = ${conditionConfiguration.noMorePausesAfterwards}`
					);
					if (conditionConfiguration.noMorePausesAfterwards) {
						...
						await breakpoints.waitAndConsumePausedEvent(
							setStateBreakpoint
						);
						await setStateBreakpoint.unset();
				...
				async () => {
					...
					try {
						...
						expect(exception.toString()).to.be.equal(
							`Error: [debugger-for-chrome] Error processing "setBreakpoints": Didn't recognize <${invalidCondition}> as a valid hit count condition`
						);
					}
				}
			);
