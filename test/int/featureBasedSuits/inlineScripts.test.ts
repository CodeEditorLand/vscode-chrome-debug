...
const SINGLE_INLINE_TEST_SPEC = TestProjectSpec.fromTestPath(
	...
	"",
	utils.pathToFileURL(
		path.join(testSetup.DATA_ROOT, "inline_scripts/single.html")
	)
);
const MULTIPLE_INLINE_TEST_SPEC = TestProjectSpec.fromTestPath(
	...
	"",
	utils.pathToFileURL(
		path.join(testSetup.DATA_ROOT, "inline_scripts/multiple.html")
	)
);
...
suite("Inline Script Tests", () => {
	puppeteerSuite(
		...
		SINGLE_INLINE_TEST_SPEC,
		(suiteContext) => {
			const frameworkTests = new FrameworkTestSuite(
				"Simple JS",
				suiteContext
			);
